...
def init(page) -> None:
    ...
    def moderators(**options) -> ResponseReturnValue:
        with DBManager.create_session_scope() as db_session:
            ...
            userlists["Súper Moderadores"] = list(filter(lambda user: 1000 <= user.level < 1999, moderator_users))
            userlists["Moderadores"] = list(filter(lambda user: 500 <= user.level < 1000, moderator_users))
